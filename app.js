...
function addMode() {
    ...
    const newMode = {
        ...
        totalGames: 0,
        counters: JSON.parse(JSON.stringify(DEFAULT_COUNTERS_DATA))
    };
...
function deleteCounter(id) {
    ...
    if (confirm(`項目「${target.name}」を削除してもよろしいですか？`)) {
        modes.forEach(m => {
            ...
            m.counters = m.counters.filter(c => c.id !== id);
        });
    ...
    counters.forEach(c => {
        ...
        card.addEventListener('drop', (e) => {
            ...
            if (draggingCard && draggingCard !== card) {
                // ドロップされた位置のカードとドラッグ中カードを入れ替える
                swapElements(draggingCard, card);
        ...
        // スマートフォン（タッチデバイス）向けのドラッグ＆ドロップサポート
        ...
        card.addEventListener('touchend', () => {
            ...
            if (targetCard && targetCard !== card) {
                // 指を離した位置のカードとドラッグ中カードを入れ替える
                swapElements(card, targetCard);
                saveCountersOrder();
            }
        ...
        if (!c.isRate) {
            ...
            PALETTE_COLORS.forEach(p => {
                const opt = document.createElement('option');
                opt.value = p.code;
                opt.textContent = '■'; // 色の名称テキストを排除し、四角のみで表現
            ...
            // サウンド選択セレクト（音符アイコンに固定表示するためのラッパー）
        ...
        nameInput.oninput = (e) => updateCounterName(c.id, e.target.value);
        ...
        if (c.isRate) {
            ...
            ratioSpan.textContent = `${numVal} / ${denVal} 回`;
            ...
            percentSpan.textContent = (c.calculatedPercent || 0).toFixed(1) + ' %';
            ...
            unitSpan.textContent = '回';
            leftContainer.appendChild(unitSpan);
        ...
        if (c.isCombined) {
            ...
            addBtn.className = 'count-btn add-btn';
            addBtn.textContent = '+1';
            
            // 各小役カードのカスタムカラーを取得（ループ外で定義されたdefaultColorsを使用）
            const currentBtnColor = c.color || defaultColors[c.id] || '#FF9100';
            addBtn.style.backgroundColor = currentBtnColor;
            
            // 明るい色（黄、ゴールドなど）の時は文字色をダークグレーに設定して視認性を担保
            if (currentBtnColor.toLowerCase() === '#ffd700' || currentBtnColor.toLowerCase() === '#ffff00' || currentBtnColor.toLowerCase() === '#ffffff') {
                addBtn.style.color = '#424242';
            } else {
                addBtn.style.color = 'white';
            }

            addBtn.onclick = () => countUp(c.id, 1);
            btnGroup.appendChild(addBtn);

            const subBtn = document.createElement('button');
            subBtn.className = 'count-btn sub-btn';
            subBtn.textContent = '-1';
            subBtn.title = '押しミス修正';
            subBtn.onclick = () => countUp(c.id, -1);
...
function updateDisplay() {
    ...
    counters.forEach(c => {
        ...
        if (c.isRate) {
            ...
            if (rateRatioEl) {
                rateRatioEl.textContent = `${numVal} / ${denVal} 回`;
            }
            if (ratePercentEl) {
                ratePercentEl.textContent = (c.calculatedPercent || 0).toFixed(1) + ' %';
            }
        }
        ...
        if (rateTextEl) {
            if (c.isRate) {
                if (c.calculatedDenominator > 0) {
                    rateTextEl.textContent = '1/ ' + c.calculatedDenominator.toFixed(1);
                } else {
                    rateTextEl.textContent = '1/ -';
                }
            } else {
                if (totalGames > 0 && c.count > 0) {
                    const autoDenominator = totalGames / c.count;
                    rateTextEl.textContent = '1/ ' + autoDenominator.toFixed(1);
                } else {
                    rateTextEl.textContent = '1/ -';
                }
            }
...
function saveHistoryLog() {
    ...
    if (maxProb > uniformVal) {
        estimationText = `設定${maxIdx + 1} (${maxProb.toFixed(1)}%)`;
    } else {
...
function loadHistoryLog(logId) {
    ...
    if (!confirm('履歴データを呼び出すと、現在カウントしている数値や状態が上書きされて消えてしまいます。よろしいですか？')) {
        return;
    }
...
function renderGallery() {
    ...
    request.onsuccess = function(e) {
        ...
        list.forEach(item => {
            ...
            zoomBtn.className = 'image-action-btn zoom';
            zoomBtn.innerHTML = '🔍';
            zoomBtn.title = '拡大表示';
            zoomBtn.onclick = (e) => {
                e.stopPropagation();
                openLightbox(item.dataUrl);
            };
            actions.appendChild(zoomBtn);

            const delBtn = document.createElement('button');
            delBtn.className = 'image-action-btn delete';
            delBtn.innerHTML = '🗑️';
            delBtn.title = '削除';
            delBtn.onclick = (e) => {
                e.stopPropagation();
...
// 2つのDOM要素ノードの位置を直接入れ替える（交換する）関数
