...
function renderTabs() {
    ...
    modes.forEach(m => {
        ...
        if (isEditMode) {
            ...
            tabBtn.ontouchstart = (e) => e.stopPropagation();
            tabBtn.ontouchend = (e) => e.stopPropagation();
        } else {
            tabBtn.onclick = () => switchMode(m.id);
            tabBtn.title = 'タップして切り替え';
        }
...
// --- 小役のカウント数直接変更 ---
function updateCounterCount(id, count) {
    const target = counters.find(c => c.id === id);
    if (!target) return;
...
// IndexedDBの画像を別の機種キーへ複製コピーするヘルパー
function copyIndexedDBImages(oldKey, newKey, callback) {
    if (!db) {
    ...
    try {
        ...
        const request = index.getAll(oldKey);
        ...
        request.onsuccess = function(e) {
            ...
            list.forEach(item => {
                ...
                const addReq = writeStore.add(newItem);
        ...
        request.onerror = function() {
            console.error("複製元の画像データの取得に失敗しました。");
            if (callback) callback();
        };
    } catch (err) {
        console.error("画像データ複製エラー:", err);
        if (callback) callback();
    }
}
...
function saveCurrentModelAsPreset() {
    ...
    if (currentModelKey.startsWith('user-preset-')) {
        ...
        // メモ帳のコピー
        if (modelMemos[oldKey] !== undefined) {
            modelMemos[presetId] = modelMemos[oldKey];
            localStorage.setItem('bright_counter_model_memos', JSON.stringify(modelMemos));
        }

        // 画像データのコピー（非同期）
        copyIndexedDBImages(oldKey, presetId, () => {
            currentModelKey = presetId;
            saveData();
            
            renderModelOptions();
            handleModelChange(presetId);
            alert(`新規機種「${presetName.trim()}」として保存しました（メモ帳と画像も引き継がれました）。`);
        });
    }
}
...
function renderCounters() {
    ...
    counters.forEach(c => {
        ...
        if (c.symbol && Array.from(c.symbol).length > 1) {
            symbolSelect.classList.add('double-symbol');
        }
        ...
        if (c.isRate) {
            ...
            if (isEditMode) {
                ...
                numSpan.title = '数値を直接編集';
                
                const handleEditClick = (e) => {
                    e.stopPropagation();
                    const inputVal = prompt(`「${c.name}」のカウント数を入力してください`, c.count);
                    if (inputVal !== null) {
                        const num = parseInt(inputVal, 10);
                        if (!isNaN(num) && num >= 0) {
                            updateCounterCount(c.id, num);
                        }
                    }
                };
                
                numSpan.onclick = handleEditClick;
                numSpan.ontouchstart = (e) => e.stopPropagation();
                numSpan.ontouchend = (e) => e.stopPropagation();
            }
...
function generateModelTable() {
    ...
    counters.forEach(c => {
        ...
        if (c.symbol) {
            ...
            if (Array.from(c.symbol).length > 1) {
                symbolSpan.classList.add('double-symbol');
            }
...
function updateDisplay() {
    ...
    counters.forEach(c => {
        ...
        // カードの is-excluded クラスの付け外しによって除外バッジの表示・非表示を切り替え（CSSの!important競合防止）
        const cardEl = document.querySelector(`.counter-card.${c.id}`);
        if (cardEl) {
            if (c.useInEstimation === false) {
                cardEl.classList.add('is-excluded');
            } else {
                cardEl.classList.remove('is-excluded');
            }
        }
    });
}
...
function exportBackup() {
    if (!confirm('現在のすべてのアプリデータ（機種、実践履歴、画像など）をバックアップファイルとしてエクスポート（ダウンロード）しますか？')) {
        return;
    }
...
function downloadJSON(data) {
    try {
        ...
        const filename = `XCBU${yyyy}${mm}${dd}.json`;
...
// 現在選択されている機種設定のみをJSONファイルとしてダウンロード
function exportCurrentModel() {
    try {
        const modelSelect = document.getElementById('model-select');
        if (!modelSelect) return;
        
        const currentModelName = modelSelect.options[modelSelect.selectedIndex].text;
        
        if (!confirm(`現在選択されている機種設定「${currentModelName}」のみを機種個別ファイルとしてエクスポート（ダウンロード）しますか？`)) {
            return;
        }
        ...
        const exportData = {
            ...
            modes: JSON.parse(JSON.stringify(modes)),
            enabledSettings: [...enabledSettings],
            memo: modelMemos[currentModelKey] || ""
        };
        ...
        // ファイル名: XC + 機種リスト名 + 日付8桁 (例: XCマイジャグラーV20260720.json)
        const now = new Date();
        const yyyy = now.getFullYear();
        const mm = String(now.getMonth() + 1).padStart(2, '0');
        const dd = String(now.getDate()).padStart(2, '0');
        ...
        // ファイル名として安全な名前に変換
        const safeModelName = currentModelName.replace(/[\s\t\n\/\\\?\*:\'\"<>\|]/g, '');
        const filename = `XC${safeModelName}${yyyy}${mm}${dd}.json`;
...
// 機種個別設定ファイルを読み込んでリストに追加し、アクティブ化する
function importModelFile(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (!data || data.type !== 'single_model') {
                alert('無効なファイル形式です。このボタンは機種個別バックアップデータ（XC...json）専用です。');
                return;
            }

            const name = data.modelName || '読み込み機種';
            if (!confirm(`機種個別設定「${name}」を機種リストに追加しますか？`)) {
                return;
            }

            // 新しい user-preset ID の発行
            const presetId = 'user-preset-' + Date.now().toString();
            userPresets[presetId] = {
                name: name.trim(),
                presetModeData: JSON.parse(JSON.stringify(data.modes)),
                enabledSettings: JSON.parse(JSON.stringify(data.enabledSettings))
            };

            // 個別メモがある場合はメモマスタに追加
            if (data.memo !== undefined) {
                modelMemos[presetId] = data.memo;
                localStorage.setItem('bright_counter_model_memos', JSON.stringify(modelMemos));
            }
            ...
            alert(`機種「${name}」を機種リストに追加し、選択しました。`);

        } catch (err) {
            alert("機種データの読み込みに失敗しました。ファイルが破損しているか、無効なファイル形式です。\nエラー詳細: " + err.message);
            console.error(err);
        }
    };
...
function importBackup(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            // 機種個別データの復元処理
            if (data.type === 'single_model') {
                if (!confirm(`機種個別バックアップデータ（${data.modelName}）を復元しますか？\n復元を実行すると、現在の小役や状態、設定差テーブルがこの機種データで上書きされます。よろしいですか？`)) {
                    return;
                }
                
                counters = JSON.parse(JSON.stringify(data.counters));
                modes = JSON.parse(JSON.stringify(data.modes));
                currentModelKey = data.modelKey || 'custom';
                enabledSettings = [...data.enabledSettings];
                
                if (data.memo !== undefined) {
                    modelMemos[currentModelKey] = data.memo;
                    localStorage.setItem('bright_counter_model_memos', JSON.stringify(modelMemos));
                }
